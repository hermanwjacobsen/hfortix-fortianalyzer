"""
FortiAnalyzer API endpoint: task.task.line
...
class TaskTaskLine:
    ...
    def get(
        ...
        if task is not None and line is not None:
            url = "/task/task/{task}/line/{line}"
            url = url.replace("{task}", task)
            url = url.replace("{line}", str(line))
        else:
            url = "/task/task/{task}/line"
            url = url.replace("{task}", task)
