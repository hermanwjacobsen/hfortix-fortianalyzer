"""
FortiAnalyzer API endpoint: cli.global.system.admin.user.adom
...
class CliGlobalSystemAdminUserAdom:
    ...
    def get(
        ...
        if user is not None and adom is not None:
            url = "/cli/global/system/admin/user/{user}/adom/{adom}"
            url = url.replace("{user}", user)
            url = url.replace("{adom}", str(adom))
        else:
            url = "/cli/global/system/admin/user/{user}/adom"
            url = url.replace("{user}", user)
    ...
    def delete(self, user: str, adom: int | str | None = None) -> FortiAnalyzerResponse:
        """
        DELETE operation.
