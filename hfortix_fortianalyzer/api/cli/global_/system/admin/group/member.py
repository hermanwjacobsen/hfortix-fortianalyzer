"""
FortiAnalyzer API endpoint: cli.global.system.admin.group.member
...
class CliGlobalSystemAdminGroupMember:
    ...
    def get(
        ...
        if group is not None and member is not None:
            url = "/cli/global/system/admin/group/{group}/member/{member}"
            url = url.replace("{group}", group)
            url = url.replace("{member}", str(member))
        else:
            url = "/cli/global/system/admin/group/{group}/member"
            url = url.replace("{group}", group)
    ...
    def delete(self, group: str, member: int | str | None = None) -> FortiAnalyzerResponse:
        """
        DELETE operation.
