"""
FortiAnalyzer API endpoint: cli.global.system.local-in-policy.dst
...
class CliGlobalSystemLocalInPolicyDst:
    ...
    def get(
        ...
        if local_in_policy is not None and dst is not None:
            url = "/cli/global/system/local-in-policy/{local-in-policy}/dst/{dst}"
            url = url.replace("{local-in-policy}", local_in_policy)
            url = url.replace("{dst}", str(dst))
        else:
            url = "/cli/global/system/local-in-policy/{local-in-policy}/dst"
            url = url.replace("{local-in-policy}", local_in_policy)
    ...
    def delete(self, local_in_policy: str, dst: int | str | None = None) -> FortiAnalyzerResponse:
        """
        DELETE operation.
