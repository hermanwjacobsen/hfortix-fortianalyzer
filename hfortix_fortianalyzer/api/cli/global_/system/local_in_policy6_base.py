"""
FortiAnalyzer API endpoint: cli.global.system.local-in-policy6
...
class CliGlobalSystemLocalInPolicy6:
    ...
    def get(
        ...
        if local_in_policy6 is not None:
            url = "/cli/global/system/local-in-policy6/{local-in-policy6}"
            url = url.replace("{local-in-policy6}", str(local_in_policy6))
        else:
            url = "/cli/global/system/local-in-policy6"
    ...
    def delete(self, local_in_policy6: int | str | None = None) -> FortiAnalyzerResponse:
        """
        DELETE operation.
