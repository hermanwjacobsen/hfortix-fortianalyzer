"""
FortiAnalyzer API endpoint: dvm.cmd.add.dev-list
...
class DvmCmdAddDevList:
    ...
    def exec(
        self,
        add_dev_list: list[Any] | None = None,
        ...
        Args:
            add_dev_list: A list of device objects to be added. Refer to <i>device action</i> attribute to select different type of add operation.
        ...
        # Build data payload
        data = {}
        if add_dev_list is not None:
            data["add-dev-list"] = add_dev_list
        if adom is not None:
            data["adom"] = adom
